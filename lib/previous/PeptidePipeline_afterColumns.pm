...
sub runPsipred{
	...
	$peptideStartPosition += 1;     #psipred expects 1-based residue numbering and ours go in as 0-based.   TODO -- see if we want to change this.  Also figure out why this wasn't problem for DSSP
	$peptideEndPosition += 1;
	...
	while (<$psipredFh>){
	    ...
	    if ($residueNumber == $peptideStartPosition){  #found cleavage sequence position, process the entire thing within the next for loop
...
sub runDisopred {
	...
	$peptideStartPosition += 1;     #disopred expects 1-based residue numbering and ours go in as 0-based.   TODO -- see if we want to change this.  Also figure out why this wasn't problem for DSSP (update: it was!)
	$peptideEndPosition += 1;
...
sub makeColumnHeaderString{
    ...
    my $peptideColumnModeName = $self->getParam("peptide_column_mode_name");

    my $columns = $self->{ColumnInfo};
    foreach my $columnName (sort ({$columns->{$a}->{displayOrder} <=> $columns->{$b}->{displayOrder}} keys %$columns)){
	my $modes = $columns->{$columnName}->{modes};   #check if the output file for Peptide Pipeline mode is supposed to output this column type
	if ($modes =~ /$peptideColumnModeName/){
	    
	    my $method = $columns->{$columnName}->{method};
...
sub loadColumnInfo{
	...
	$columnInfo->{$columnName}->{shortName} = $columnShortName;
	$columnInfo->{$columnName}->{modes} = $columnModes;
	$columnInfo->{$columnName}->{method} = $method;
	$columnInfo->{$columnName}->{description} = $columnDescription;
	$columnInfo->{$columnName}->{displayOrder} = $counter;
	$counter++;
    }
