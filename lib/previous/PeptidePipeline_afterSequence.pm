...
sub runDisopred {
	...
	$peptideStartPosition += 1;     #disopred expects 1-based residue numbering and ours go in as 0-based.   TODO -- see if we want to change this.
	$peptideEndPosition += 1;
...
sub runDisopredCommand{
    ...
    my $cpCmd = "cp $shortSeqId.horiz_d $shortSeqId.diso $outputDir";

    $self->writeLog("copy disopred results with $cpCmd");
...
sub loadColumnInfo{
	...
	$columnInfo->{$columnName}->{method} = $columnMethod;
