...
sub writeTestSetFile{
	...
	foreach my $startPosition (keys %$startPositions){
	    ...
	    my $outputLine = "";
	    unless ($allFeatureStrings){
		print STDERR "ERROR: Did not find sequence id $sequenceId P1 position $startPosition, retrieved from  test set, in data loaded from Protease Pipeline results\n";
		exit(1);
	    }
	    my @featureStringArray = sort({$a <=> $b} values %$allFeatureStrings);
